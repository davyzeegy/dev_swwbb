const websocket = new WebSocket("ws://192.168.178.30:5000");
...
function startCall() { // deze functie word aangeroepen zodra de caller op "Start gesprek" klikt
    ...
    navigator.getUserMedia({ //deze functie handelt onder andere de permissions af zoals "mag ik je microphone en camera gebruiken"
        ...
        peerConnection.onaddstream = (e) => {  //zodra de peerConnection verbinding heeft met de andere kant, dan geeft ie een callback function
            document.getElementById("remote-video").srcObject = e.stream // met e.stream kunnen we de stream 'uitpakken', en we willen het dus laten zien in de remote video element. Met de srcObject kun je het dus iets anders op dat object toewijzen.
        }
        

        peerConnection.onicecandidate = ((e) =>{ //deze onicecandidate word meerdere keren aangeroepen
        if (e.candidate == null)
            return //return niks
...
function createAndSendOffer(){ //zie implementatie functie voor beschrijving
    ...
    peerConnection.setLocalDescription(offer) // de local descriptor word hiermee geset
